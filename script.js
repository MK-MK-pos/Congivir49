document.addEventListener('DOMContentLoaded', function() {
    ...
    // Локальный рейтинг
    let localRatings = JSON.parse(localStorage.getItem('bm_ratings') || '{"total": 0, "sum": 0, "average": 0, "voters": {}}');
    let userRating = 0;
    let userHasRated = false;

    // Глобальный рейтинг (симуляция)
    let globalRatings = {
        total: 1274,
        sum: 10235,
        average: 8.0
    ...
    // Получение IP пользователя (упрощенная версия)
    async function getUserIP() {
        try {
            ...
            // Если API не работает, генерируем случайный IP для демо
            return 'demo_ip_' + Math.floor(Math.random() * 1000000);
        }
    }
    ...
    function checkUserVote() {
        ...
        userHasRated = localRatings.voters && localRatings.voters[userId];
        if (userHasRated) {
    ...
    function initializeRating() {
        avgRating.textContent = localRatings.average.toFixed(1);
        totalRatings.textContent = localRatings.total;
    ...
    // Функция рейтинга с обновлением глобального рейтинга
    async function setRating(rating) {
        ...
        // Сохраняем локальный рейтинг
        localRatings.total += 1;
        localRatings.sum += rating;
        localRatings.average = localRatings.sum / localRatings.total;
        
        // Сохраняем информацию о голосовавшем
        if (!localRatings.voters) {
            localRatings.voters = {};
        }
        localRatings.voters[userId] = {
            rating: rating,
            timestamp: Date.now()
        };
        
        localStorage.setItem('bm_ratings', JSON.stringify(localRatings));
        
        // Обновляем глобальный рейтинг (симуляция)
        ...
        // Симуляция отправки на сервер (в реальном приложении здесь был бы fetch запрос)
        try {
            const userIP = await getUserIP();
            console.log(`Rating ${rating} submitted from IP: ${userIP}`);
            
            // В реальном приложении здесь был бы код для отправки на сервер:
            // await fetch('/api/rating', {
            //     method: 'POST',
            //     headers: { 'Content-Type': 'application/json' },
            //     body: JSON.stringify({ rating, ip: userIP, timestamp: Date.now() })
            // });
            
        } catch (error) {
            console.log('Rating submitted locally');
        }
    }
